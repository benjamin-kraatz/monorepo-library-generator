import { KyselyService } from "@custom-repo/provider-kysely";
...
describe("User Repository", () => {
   ...
   *     }
   *   }).pipe(Effect.provide(UserRepositoryLive.pipe(Layer.provide(KyselyServiceMock))))
   * );
   *
   ...
   *
   *     expect(result._tag).toBe("UserRepositoryError");
   *   }).pipe(Effect.provide(UserRepositoryLive.pipe(Layer.provide(mockError))))
   * );
   */
