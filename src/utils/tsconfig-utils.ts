...
export async function computeProjectReferences(
  tree: Tree,
  projectName: string
): Promise<{ references: Array<ProjectReference>; dependencies: Array<DependencyInfo> }> {
  try {
...
export function generateLibTsConfig(
  ...
  const types = libraryType === "contract"
    ? []
 ...
 */
export function getOffsetFromRoot(projectRoot: string): string {
