...
describe("provider generator", () => {
  ...
  describe("📁 File Generation Tests", () => {
    it("should generate all required files for server-only provider", async () => {
      ...
      // Service implementation files (provider.md lines 106-113)
      expect(tree.exists(`${projectRoot}/src/lib/service/interface.ts`)).toBeTruthy()
      expect(
        tree.exists(`${projectRoot}/src/lib/service.spec.ts`)
      ).toBeTruthy()
      expect(tree.exists(`${projectRoot}/src/lib/layers.ts`)).toBeTruthy()
      ...
      // Edge should NOT be generated for node platform
      expect(tree.exists(`${projectRoot}/src/edge.ts`)).toBeFalsy()

      // Should NOT have __tests__ directory (provider.md line 137)
      expect(tree.exists(`${projectRoot}/src/lib/__tests__`)).toBeFalsy()

      // Should NOT have separate interface.ts (inline interface pattern)
      expect(tree.exists(`${projectRoot}/src/lib/interface.ts`)).toBeFalsy()
    })

    it("should generate client.ts for universal platform with includeClientServer", async () => {
      await providerGenerator(tree, {
        name: "analytics",
    ...
    it("should generate edge.ts for edge platform", async () => {
      await providerGenerator(tree, {
        name: "auth",
      ...
      // Platform barrel exports are ALWAYS generated
      expect(tree.exists(`${projectRoot}/src/server.ts`)).toBeTruthy()
      expect(tree.exists(`${projectRoot}/src/client.ts`)).toBeTruthy()
    })
  })
  ...
  describe("📦 Package.json Tests", () => {
    it("should create package.json with correct structure", async () => {
      ...
      // For node platform, only server export is generated by default
      expect(packageJson.exports?.["./server"]).toBeDefined()
      // Client export is not generated for node platform without includeClientServer
  ...
  describe("📝 Template Content Tests", () => {
    it("should use Context.Tag pattern in service interface", async () => {
      ...
      // New granular structure: service/interface.ts
      const serviceContent = tree.read(
        "libs/provider/stripe/src/lib/service/interface.ts",
        "utf-8"
      )
    ...
    it("should include all Layer types in layers.ts", async () => {
      await providerGenerator(tree, {
        name: "stripe",
        externalService: "Stripe API",
        platform: "node"
      ...
      // Layer types (provider.md lines 1548-1587)
      expect(layersContent).toContain("Layer.succeed")
      expect(layersContent).toContain("Layer.sync")
      expect(layersContent).toContain("Layer.effect")
      expect(layersContent).toContain("Layer.scoped")

      // Layer implementations
      expect(layersContent).toContain("StripeLive")
      expect(layersContent).toContain("StripeDev")
      expect(layersContent).toContain("StripeTest")
      expect(layersContent).toContain("StripeAuto")
    })

    it("should export service implementation in client.ts when includeClientServer is true", async () => {
      await providerGenerator(tree, {
        name: "analytics",
        externalService: "Analytics Service",
        platform: "universal",
  ...
  describe("🔧 TypeScript Configuration Tests", () => {
    it("should enable composite and incremental compilation", async () => {
      ...
      // tsBuildInfoFile should be in dist/<projectRoot>/, not in a .tsbuildinfo/ directory
      expect(tsconfigLib.compilerOptions?.tsBuildInfoFile).toContain(
        "dist/libs/provider/stripe"
      )
      expect(tsconfigLib.compilerOptions?.tsBuildInfoFile).not.toContain(
        ".tsbuildinfo/"
      )

      // outDir should be in dist/<projectRoot>/
      expect(tsconfigLib.compilerOptions?.outDir).toContain(
        "dist/libs/provider/stripe"
      )
    })
    ...
    it("should include source files in base tsconfig.json", async () => {
      ...
      const tsconfig = JSON.parse(
        tree.read("libs/provider/stripe/tsconfig.json", "utf-8") || "{}"
      )
  ...
  describe("🎯 Naming Convention Tests", () => {
    it("should use correct naming transformations", async () => {
      ...
      const serviceContent = tree.read(
        "libs/provider/my-custom-service/src/lib/service/interface.ts",
        "utf-8"
      )
