...
function getStructurePath(
  structure: (typeof LIBRARY_STRUCTURES)[keyof typeof LIBRARY_STRUCTURES] | undefined,
  key: string
): string | undefined {
...
function generatePackageJson(
  ...
  const scopedName = `@creativetoolkits/${options.projectName}`
  ...
  const needsEffectVitest = ["provider", "infra", "feature"].includes(options.libraryType)
 ...
 *
 * ${options.description || `${nameVars.className} library for ${options.projectName}`}
 */
...
function generateReadmeTemplate(
  ...
  return `# @creativetoolkits/${options.projectName}
...
\`\`\`bash
pnpm add @creativetoolkits/${options.projectName}
\`\`\`

## Usage

\`\`\`typescript
import { ${nameVars.className} } from '@creativetoolkits/${options.projectName}';
\`\`\`
...
${
    options.libraryType !== "data-access" && options.libraryType !== "contract" &&
      ...
      ? `// Server exports\nimport { ${nameVars.className}Live } from '@creativetoolkits/${options.projectName}/server';\n`
      : ""
  }
${
    options.libraryType !== "data-access" && options.libraryType !== "contract" &&
      (options.includeClientServer || options.platform === "browser" || options.platform === "universal")
      ? `// Client exports\nimport type { ${nameVars.className}Type } from '@creativetoolkits/${options.projectName}/client';\n`
      : ""
  }
\`\`\`
...
import { ${nameVars.className}Service } from '@creativetoolkits/${options.projectName}/server';
...
2. pnpm workspace resolves \`@creativetoolkits/*\` packages
...
function generateVitestConfig(
  ...
  const needsEffectSetup = ["provider", "infra", "feature"].includes(options.libraryType)
