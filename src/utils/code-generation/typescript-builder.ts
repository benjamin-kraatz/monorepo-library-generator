...
export class TypeScriptBuilder {
  ...
  addClass(config: ClassConfig): this {
    ...
    if (config.fields && config.fields.length > 0) {
      for (const field of config.fields) {
        ...
        this.lines.push(`  ${visibility} ${readonlyModifier}${field.name}${optionalModifier}: ${field.type};`)
      }
      this.lines.push("")
    }
  ...
  private addMethodToClass(method: MethodConfig, isStatic: boolean): void {
    ...
    this.lines.push(`  ${visibilityPrefix}${staticModifier}${asyncModifier}${method.name}(${params})${returnType} {`)
  ...
  addFunction(config: FunctionConfig): this {
    ...
    this.lines.push(`${exported}${asyncModifier}function ${config.name}${typeParams}(${params})${returnType} {`)
   ...
   */
  addConst(name: string, value: string, type?: string, exported = true, jsdoc?: string): this {
    if (jsdoc) {
      this.addJSDoc(jsdoc)
    }
