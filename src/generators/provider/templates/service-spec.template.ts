 ...
 */
export function generateServiceSpecFile(options: ProviderTemplateOptions): string {
  ...
  builder.addRaw(" *")
  builder.addRaw(` * Tests verify that the provider correctly wraps the ${externalService} SDK with Effect patterns.`)
  builder.addRaw(" * Uses @effect/vitest with minimal inline mocking for rapid iteration.")
  builder.addRaw(" *")
  builder.addRaw(" * Testing Guidelines:")
  builder.addRaw(" * - ✅ Test SDK wrapping (does the provider correctly wrap SDK methods?)")
  builder.addRaw(" * - ✅ Use it.effect for simple provider tests")
  builder.addRaw(" * - ✅ Create inline mocks with minimal test data")
  builder.addRaw(" * - ✅ Focus on error transformation and Effect integration")
  ...
  builder.addRaw(" * - ❌ DON'T test the external SDK itself (that's the SDK's responsibility)")
  builder.addRaw(" * - ❌ DON'T create complex mock objects matching full SDK types")
  builder.addRaw(" * - ❌ DON'T use manual Effect.runPromise (use it.effect instead)")
  builder.addRaw(" */")
  ...
  builder.addRaw("  /**")
  builder.addRaw(`   * TODO: Implement tests for your ${externalService} service`)
  builder.addRaw("   *")
  ...
  builder.addRaw("   *     const result = yield* service.createResource({ data: \"test\" });")
  ...
  builder.addRaw("   *   Effect.gen(function* () {")
  builder.addRaw(`   *     const service = yield* ${className};`)
  builder.addRaw("   *     const result = yield* service.failingMethod().pipe(Effect.flip);")
