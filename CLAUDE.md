...
This is a **dual-mode monorepo generator** for creating standardized Effect-based library architectures. It supports both:

- **Nx monorepos** - Full Nx integration with project.json and Nx executors
- **Effect-native monorepos** - Pure pnpm workspaces with @effect/build-utils

The plugin provides five core generators (contract, data-access, feature, infra, provider) that create libraries following Effect.ts patterns with type-safe service definitions, layers, and error handling.
