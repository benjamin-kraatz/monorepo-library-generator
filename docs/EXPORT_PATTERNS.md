...
```typescript
// SERVER (Node.js):
//   import { ProductService } from '@custom-repo/feature-product/server';
//
// CLIENT (Browser):
//   import { useProduct } from '@custom-repo/feature-product/client';
//
// EDGE (Cloudflare Workers, Vercel Edge):
//   import { productMiddleware } from '@custom-repo/feature-product/edge';
```
...
**Platform:** Typically server-only (unless browser SDK exists)
...
// Layer Compositions
export * from "./lib/layers";  // StripeServiceLive, StripeServiceTest, StripeServiceAuto
```

**Usage Pattern:**
```typescript
import { StripeService } from '@custom-repo/provider-stripe';
...
**No platform-specific exports** - Providers are typically server-only.
...
```typescript
// OLD (pre-3.0)
import { ProductServiceLive } from '@custom-repo/feature-product';
...
import type { Product, ProductRepository } from '@custom-repo/contract-product';
import { ProductNotFoundError } from '@custom-repo/contract-product';
```

### Data-Access Library
```typescript
// Repository implementation (server-only)
import { ProductRepository } from '@custom-repo/data-access-product';
...
// Universal types
import type { ProductInput } from '@custom-repo/feature-product';
```
...
import { useCache } from '@custom-repo/infra-cache/client';
```
...
2. **Use type-only imports** when possible for better tree-shaking:
   ```typescript
   import type { Product } from '@custom-repo/contract-product';
   ```
...
1. **Don't mix client and server imports:**
   ...
   import { ProductService } from '@custom-repo/feature-product';
   ```
...
Platform-specific exports are determined by `platform-utils.ts:determinePlatformExports()`.
