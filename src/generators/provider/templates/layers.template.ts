...
export function generateLayersFile(options: ProviderTemplateOptions): string {
  ...
  builder.addImport("@creativetoolkits/infra-env", "env")
  ...
  builder.addRaw(" *")
  builder.addRaw(" * Configuration: Uses @creativetoolkits/infra-env for environment variable access")
  builder.addRaw(" *")
  builder.addRaw(" * TODO: If your SDK requires async initialization, use Layer.effect instead")
  builder.addRaw(
  ...
  builder.addRaw("  () => {")
  builder.addRaw(`    const config: ${className}Config = {`)
  builder.addRaw(`      apiKey: env.${projectConstantName}_API_KEY || "dev_key",`)
  ...
  builder.addRaw("    {")
  builder.addRaw("      healthCheck: () => Promise.resolve({ status: \"healthy\" as const }),")
  builder.addRaw("    },")
  builder.addRaw("    {")
