 ...
 */
export function generateServiceFile(options: ProviderTemplateOptions): string {
  ...
  builder.addRaw(" *")
  builder.addRaw(` * Provider: External service adapter for ${externalService}`)
  builder.addRaw(" * Pattern: Context.Tag with inline interface (NOT Context.GenericTag)")
  ...
  builder.addRaw(" *")
  builder.addRaw(" * CRITICAL: Use Context.Tag with inline interface (NOT Context.GenericTag)")
  ...
  builder.addRaw(" * ```")
  builder.addRaw(" */")
  builder.addRaw(`export class ${className} extends Context.Tag("${className}")<`)
  builder.addRaw(`  ${className},`)
  builder.addRaw("  {")
  builder.addRaw("    /**")
  builder.addRaw("     * Health check - verifies service connectivity")
  builder.addRaw("     */")
  builder.addRaw(`    readonly healthCheck: Effect.Effect<HealthCheckResult, ${className}Error>;`)
  builder.addBlankLine()
  builder.addRaw("    /**")
  ...
  builder.addRaw("     *")
  builder.addRaw("     * @param params - Query parameters including pagination")
  ...
  builder.addRaw("     * const result = yield* service.list({ page: 1, limit: 10 });")
  builder.addRaw("     * ```")
  builder.addRaw("     */")
  builder.addRaw("    readonly list: (")
  builder.addRaw("      params?: ListParams")
  builder.addRaw(`    ) => Effect.Effect<PaginatedResult<Resource>, ${className}Error>;`)
  builder.addBlankLine()
  builder.addRaw("    /**")
  ...
  builder.addRaw("     * @example")
  builder.addRaw("     * ```typescript")
  builder.addRaw("     * const created = yield* service.create({ name: \"example\" });")
  builder.addRaw("     * ```")
  builder.addRaw("     */")
  ...
  builder.addRaw("     * @example")
  builder.addRaw("     * ```typescript")
  builder.addRaw("     * const updated = yield* service.update(\"id\", { name: \"new name\" });")
  builder.addRaw("     * ```")
  builder.addRaw("     */")
  ...
  builder.addRaw("     * @example")
  builder.addRaw("     * ```typescript")
  builder.addRaw("     * const results = yield* service.search(\"query\", { status: \"active\" });")
  builder.addRaw("     * ```")
  builder.addRaw("     */")
  builder.addRaw("    readonly search: (")
  builder.addRaw("      query: string,")
  builder.addRaw("      filters?: Record<string, unknown>")
  builder.addRaw(`    ) => Effect.Effect<readonly Resource[], ${className}Error>;`)
  builder.addRaw("  }")
  builder.addRaw(">() {")
  builder.addRaw("  /**")
  ...
  builder.addRaw(`    // Example: const sdkClient = client as ${className}SDK;`)
  ...
  builder.addRaw("    const retryPolicy = Schedule.exponential(Duration.millis(serviceConfig.retryDelay))")
  builder.addRaw("      .pipe(Schedule.compose(Schedule.recurs(serviceConfig.retryAttempts)));")
  builder.addBlankLine()
  ...
  builder.addRaw("        try: async (): Promise<PaginatedResult<Resource>> => {")
  ...
  builder.addRaw("    const update = (id: string, data: UpdateResourceData) =>")
  builder.addRaw("      Effect.tryPromise({")
  builder.addRaw("        try: async (): Promise<Resource> => {")
  builder.addRaw("          // TODO: Replace with actual SDK call")
  builder.addRaw("          // Example: return await sdkClient.update(id, data);")
  builder.addBlankLine()
  builder.addRaw("          return { id, data };")
  builder.addRaw("        },")
  builder.addRaw(`        catch: map${className}Error,`)
  ...
  builder.addRaw("    const search = (query: string, filters?: Record<string, unknown>) =>")
  builder.addRaw("      Effect.tryPromise({")
  builder.addRaw("        try: async (): Promise<readonly Resource[]> => {")
  builder.addRaw("          // TODO: Replace with actual SDK call")
  builder.addRaw("          // Example: return await sdkClient.search({ query, ...filters });")
  builder.addBlankLine()
  builder.addRaw("          return [];")
  builder.addRaw("        },")
  builder.addRaw(`        catch: map${className}Error,`)
  ...
  builder.addInterface({
    ...
    properties: [
      { name: "apiKey", type: "string", readonly: true, jsdoc: "API key for authentication" },
      { name: "timeout", type: "number", readonly: true, optional: true, jsdoc: "Request timeout in milliseconds" },
      { name: "retryAttempts", type: "number", readonly: true, optional: true, jsdoc: "Number of retry attempts" },
      {
        name: "retryDelay",
        type: "number",
        readonly: true,
  ...
  builder.addRaw("  // TODO: Add additional configuration options as needed (e.g., baseUrl, region)")
  builder.addBlankLine()
  ...
  builder.addInterface({
    ...
    properties: [
      ...
      { name: "timestamp", type: "string", readonly: true, jsdoc: "Check timestamp" },
      { name: "version", type: "string", readonly: true, jsdoc: "Service version" },
      { name: "details", type: "Record<string, unknown>", readonly: true, optional: true, jsdoc: "Additional details" }
    ]
  })
  ...
  builder.addFunction({
    ...
    params: [
      { name: "config", type: `${className}Config` }
    ],
    returnType: "unknown",
...
return {
  ...
  builder.addInterface({
    ...
    properties: [
      { name: "page", type: "number", readonly: true, optional: true, jsdoc: "Page number" },
      { name: "limit", type: "number", readonly: true, optional: true, jsdoc: "Items per page" },
      { name: "filters", type: "Record<string, unknown>", readonly: true, optional: true, jsdoc: "Filter criteria" }
    ]
  })
  ...
  builder.addInterface({
    ...
    properties: [
      { name: "data", type: "readonly T[]", readonly: true, jsdoc: "Result data" },
      { name: "total", type: "number", readonly: true, jsdoc: "Total count" },
      { name: "page", type: "number", readonly: true, jsdoc: "Current page" },
      { name: "limit", type: "number", readonly: true, jsdoc: "Items per page" },
      { name: "hasMore", type: "boolean", readonly: true, jsdoc: "More results available" }
    ]
  })
  ...
  builder.addInterface({
    ...
    properties: [
      ...
      { name: "data", type: "Record<string, unknown>", readonly: true, jsdoc: "Resource data" }
    ]
  })
  ...
  builder.addInterface({
    ...
    exported: true,
    jsdoc: "Resource creation data\n\nTODO: Define actual creation data structure",
    properties: [
      { name: "[key: string]", type: "unknown", jsdoc: "Dynamic creation fields" }
    ]
  })
  ...
  builder.addInterface({
    ...
    properties: [
      { name: "[key: string]", type: "unknown", jsdoc: "Dynamic update fields" }
    ]
  })
